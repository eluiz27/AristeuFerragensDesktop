...
namespace processosAdministrativos.Classes
{
    class DAO
    {
        private static MySqlConnection conexao = new MySqlConnection("server=10.0.0.210;userid=Luiz;password=GEN@#0996;database=gs_aristeus");
        ...
        public void cadastraEntrega(controlEntreg cl)
        {
            ...
            Query.Parameters.AddWithValue("@dt_expedicao", cl.Dt_expedicao);
            Query.Parameters.AddWithValue("@dt_aCaminho", cl.Dt_aCaminho);
            Query.Parameters.AddWithValue("@dt_entregue", cl.Dt_entregue);
            Query.CommandText = "INSERT INTO controle_entrega (pedido_contEntre, vendedor_contEntre, cliente_contEntre, motoboy_contEntre, situacao_contEntre, dt_expedicao, dt_aCaminho, dt_entregue) VALUES (@pedido_contEntre, @vendedor_contEntre, @cliente_contEntre, @motoboy_contEntre, @situacao_contEntre, @dt_expedicao, @dt_aCaminho, @dt_entregue)";
            conecta();
            Query.ExecuteNonQuery();
            desconecta();
        }
        public void alteraEntrega(controlEntreg cl)
        {
            Query = new MySqlCommand();
            Query.Connection = Conexao;
            Query.Parameters.AddWithValue("@situacao_contEntre", cl.Situacao_contEntre);
            ...
            Query.Parameters.AddWithValue("@dt_aCaminho", cl.Dt_aCaminho);
            Query.Parameters.AddWithValue("@dt_entregue", cl.Dt_entregue);
            Query.CommandText = "UPDATE controle_entrega SET motoboy_contEntre = @motoboy_contEntre, situacao_contEntre = @situacao_contEntre, dt_expedicao = @dt_expedicao, dt_aCaminho = @dt_aCaminho, dt_entregue = @dt_entregue WHERE pedido_contEntre =" + cl.Alterar;
            conecta();
            Query.ExecuteNonQuery();
            desconecta();
        }
